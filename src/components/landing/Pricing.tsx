  ...
  <div className={`bg-zinc-900 rounded-lg border ${popular ? 'border-gold-DEFAULT ring-2 ring-gold-DEFAULT/20' : 'border-zinc-800'} shadow-sm p-6 relative transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${popular ? 'hover:shadow-gold-DEFAULT/30' : 'hover:shadow-zinc-800/30'}`}>
    {popular && (
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <span className="bg-gradient-to-r from-gold-DEFAULT to-gold-light text-black text-xs font-semibold px-4 py-1 rounded-full">
          Most popular
        </span>
      </div>
    )}
    <div className="text-center">
      ...
      <div className="flex justify-center items-baseline mb-4">
        <span className={`text-4xl font-bold ${popular ? 'text-gold-DEFAULT' : 'text-white'}`}>{price}</span>
        <span className="text-gray-500 ml-1">/month</span>
      </div>
      <Button className={`w-full mb-6 ${popular ? 'bg-gradient-to-r from-gold-DEFAULT to-gold-light hover:from-gold-light hover:to-gold-DEFAULT text-black' : 'bg-zinc-800 hover:bg-zinc-700'}`}>
        {buttonText}
      </Button>
    </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16 animate-fade-in">
          <div className="inline-block px-4 py-1.5 bg-zinc-800/80 backdrop-blur-sm rounded-full border border-zinc-700 mb-6">
            <span className="text-sm text-gold-DEFAULT uppercase font-medium tracking-wider">Pricing</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            Simple, transparent <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold-DEFAULT to-gold-light">pricing</span>
          </h2>
