    ...
    <div className={`bg-zinc-900 rounded-lg border ${popular ? 'border-gold ring-2 ring-gold-opacity-20' : 'border-zinc-800'} shadow-sm p-6 relative transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${popular ? 'hover:shadow-gold-opacity-30' : 'hover:shadow-zinc-800/30'}`}>
      ...
      <div className="text-center">
        ...
        <div className="flex justify-center items-baseline mb-4">
          <span className={`text-4xl font-bold ${popular ? 'text-gold' : 'text-white'}`}>{convertPrice(price)}</span>
          <span className="text-gray-500 ml-1">/month</span>
        </div>
        <Button className={`w-full mb-6 ${popular ? 'bg-gradient-to-r from-gold to-gold-light hover:from-gold-dark hover:to-gold text-gold-foreground' : 'bg-zinc-800 hover:bg-zinc-700'}`}>
          {buttonText}
        </Button>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16 animate-fade-in">
          <div className="inline-block px-4 py-1.5 bg-zinc-800/80 backdrop-blur-sm rounded-full border border-zinc-700 mb-6">
            <span className="text-sm text-gold uppercase font-medium tracking-wider">{t("Pricing")}</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            {t("Simple, transparent")} <span className="text-transparent bg-clip-text bg-gradient-to-r from-gold to-gold-light">{t("pricing")}</span>
          </h2>
